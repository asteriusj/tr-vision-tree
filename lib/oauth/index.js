...
function GetOAuthClient(req) {
  ...
  // if there is no access_token or its empty (could be expired or not initially requested yet)
  if (!req.session.oauth_access_token) {
    console.log("requesting refresh of access_token");
    GoogleOAuth2Client.refreshToken_(req.session.oauth_refresh_token, function(err, result) {
      if (err || (result && !result.access_token)) {
        console.error('error requesting access token', err);
      }
      req.session.oauth_access_token = result.access_token;
    });
  }
...
exports.RequireOAuth = function(req, res, next) {
  ...
  if (!req.session.oauth_access_token) {
    console.log('no oauth token in session');
    // set tokens from config
    console.log('setting oauth tokens from config');
